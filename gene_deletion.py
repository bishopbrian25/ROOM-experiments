...
# Import solving modules
import fba_test
import room_test
import room

def smart_output_results(gene_dict, algorithm):
    writer = csv.writer(open(model.name + "_" + algorithm + '.csv', 'w+'))
    # Header for the table
    writer.writerow(['no.', 'gene', 'biomass'])
    i = 1 # Gives each gene deletion a number
    for gene, flux_percent in gene_dict.iteritems():
        write_line = [" ", i]
        for fluxes in flux_percent:
            write_line += [fluxes]
...
def run_tests(algorithm):
    ...
    for exp in medium_ids:
        ...
        # Every other exp we give both O2 and CO2
        else:
            mm.limits['R_EX_o2_e_'].lower = -2.0
        ...
        # Run the test
        if algorithm == "fba":
            simulation = dict(fba_test.gene_deletion(model, mm, solver))
        elif algorithm == "room":
            simulation = dict(room_test.gene_deletion(model, mm, solver, True))
...
def calc_stat(gene_dictionary):
    medium_names = ["Glucose (aerobe)",  "Galactose", "Glycerol", "Ethanol"]
    tests = [0, 1, 2, 3]
    medium_dictionary = {}
    for medium in tests:
        true_live = 0
        ...
        for gene, flux in gene_dictionary.iteritems():
            if flux[medium] == flux[medium + 4]:
                ...
                if flux[medium] == 0:
                    ...
                    false_live += 1
        medium_dictionary[medium_names[medium]] = [true_live, false_live, true_death, false_death]
...
def comparing(all_files):
    ...
    for compare_file in all_files:
        compare_exp = read_gene_file("paper_exp.csv", [2, 3, 5, 6, 7])
        my_exp = read_gene_file(compare_file, [2, 3, 4, 5, 6])
        my_exp = _round(my_exp, 0.20)
        ...
        writer2.writerow(["Algorithm ","Medium", "True Alive", "True Death", "False Alive",  "False Dead", "Percent Alive", "Percent Dead", "MCC"])
        for medium, results in medium_dictionary.iteritems():
            ...
            writer2.writerow([compare_file, medium, TP, TN, FP, FN, float(TP/(TP+FN)), float(TN/(TN + FP)), mcc])
    return medium_dictionary
